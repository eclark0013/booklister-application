      ...
      return (
        <div>          
          <Navbar style={{marginBottom: "1em"}} bg="dark" variant="dark" sticky="top">
            <Navbar.Brand as={Link} to="/">Booklister</Navbar.Brand>
            <NavDropdown title="Lists" id="collapsible-nav-dropdown">
              <NavDropdown.Item eventKey={0} as={Link} to="/lists">All Lists</NavDropdown.Item>
              <NavDropdown.Divider />
              {this.props.lists.map( list => 
                <NavDropdown.Item key={list.id} as={Link} to={`/lists/${list.id}`}>{list.name}</NavDropdown.Item>
              )}
            </NavDropdown>
            <Nav.Link as={Link} to="/lists/new">New List</Nav.Link>
            <NavDropdown title="Books" id="collapsible-nav-dropdown">
              ...
              {this.props.all_books_list.books.map( book => 
                <NavDropdown.Item key={book.id} as={Link} to={`/books/${book.id}`}>{book.title}</NavDropdown.Item>
              )}
            </NavDropdown>
            <Nav.Link as={Link} to="/books/new">New Book</Nav.Link>
          </Navbar>
          <Switch>
